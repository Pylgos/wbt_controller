...
#error                                                                         \
    "Missing declaration of LIBCONTROLLER_VERSION preprocessor macro in the Makefile."
#endif
...
static void robot_configure(WbRequest *r) {
  ...
  if (strlen(WEBOTS_VERSION) &&
      (strncmp(WEBOTS_VERSION, LIBCONTROLLER_VERSION, 6)))
    fprintf(stderr,
            "Warning: Webots [%s] and libController [%s] versions are not the "
            "same for Robot '%s'! Different versions can lead "
...
void robot_read_answer(WbDevice *d, WbRequest *r) {
  ...
  case C_ROBOT_NEW_DEVICE:
    ...
    robot.device =
        realloc(robot.device, sizeof(WbDevice *) * (robot.n_device + n));
    if (!robot.device) {
      fprintf(stderr,
              "Error initializing the new device: not enough memory.\n");
      exit(EXIT_FAILURE);
    }
  ...
  case C_ROBOT_WWI_MESSAGE:
    ...
    if (robot.wwi_reception_buffer_size < new_size) {
      robot.wwi_reception_buffer =
          realloc(robot.wwi_reception_buffer, new_size);
      robot.wwi_reception_buffer_size = new_size;
    }
    memcpy(robot.wwi_reception_buffer + robot.wwi_received_messages_size,
           request_read_data(r, n), n);
...
int robot_check_supervisor(const char *func_name) {
  ...
  fprintf(stderr,
          "Error: ignoring illegal call to %s() in a 'Robot' controller.\n",
          func_name);
  fprintf(
      stderr,
...
void robot_mutex_lock() { wb_robot_mutex_lock(robot_step_mutex); }

void robot_mutex_unlock() { wb_robot_mutex_unlock(robot_step_mutex); }
...
bool robot_is_immediate_message() { return robot.is_immediate_message; }
...
#endif
  // cppcheck-suppress resourceLeak ; for thread_handle (which we don't need
  // anymore)
}
...
static int robot_step_begin(int duration) {
  if (waiting_for_step_end)
    fprintf(stderr,
            "Warning: %s() called multiple times before calling "
            "wb_robot_step_end().\n",
            __FUNCTION__);
...
int wb_robot_step(int duration) {
  ...
  if (waiting_for_step_end)
    fprintf(stderr,
            "Warning: %s() called before calling wb_robot_step_end().\n",
            __FUNCTION__);
...
WbUserInputEvent wb_robot_wait_for_user_input_event(WbUserInputEvent event_type,
  ...
  if (event_type & (WB_EVENT_MOUSE_CLICK | WB_EVENT_MOUSE_MOVE)) {
    if (wb_mouse_get_sampling_period() <= 0)
      fprintf(stderr,
              "Error: %s() called with an event type including the mouse, but "
              "the mouse is disabled, please enable it with "
              "wb_mouse_enable().\n",
              __FUNCTION__);
    else
      valid = true;
  ...
  if (event_type & WB_EVENT_KEYBOARD) {
    if (wb_keyboard_get_sampling_period() <= 0)
      fprintf(stderr,
              ...
              "but the keyboard is disabled, please enable it "
  ...
  if (event_type & (WB_EVENT_JOYSTICK_BUTTON | WB_EVENT_JOYSTICK_AXIS |
    ...
    if (wb_joystick_get_sampling_period() <= 0)
      fprintf(stderr,
              "Error: %s() called with an event type including a joystick, but "
              "no joystick is enabled, please enable it with "
              "wb_joystick_enable().\n",
              __FUNCTION__);
    else
      valid = true;
...
void wb_robot_flush_unlocked(const char *function) {
  if (function && waiting_for_step_end) {
    fprintf(stderr,
            "Warning: %s(): functions with immediate requests to Webots cannot "
            "be implemented in-between wb_robot_step_begin() and "
            "wb_robot_step_end()!\n",
            function);
    return;
  }
...
static char *encode_robot_name(const char *robot_name, int chars_used) {
  ...
  // the robot name is used to connect to the libController and in this process
  // there are indirect limitations such as QLocalServer only accepting strings
  // up to 91 characters long for server names on some platforms. Since the
  // server name also contains the tmp path and that includes the user's
  // username and, in the case of a Snap, the user's home directory, we need to
  // limit the length of the encoded robot name based on the tmp path. If it is
  // longer than that, then we compute a hashed version of the name and use as
  // much of it as we can. If that would be less than 4 chars, we try to use 4
  // chars and hope we are on a platform where QLocalServer accepts longer
  // names. 4 chars makes the chance of a name collision 1/65536. Note: It is
  // critical that the same logic is used in WbRobot.cpp
...
static char *compute_socket_filename(char *error_buffer) {
  ...
  if (WEBOTS_ROBOT_NAME && WEBOTS_ROBOT_NAME[0] && WEBOTS_INSTANCE_PATH &&
      WEBOTS_INSTANCE_PATH[0]) {
    robot_name =
        encode_robot_name(WEBOTS_ROBOT_NAME,
                          strlen(WEBOTS_INSTANCE_PATH) + strlen("ipc//intern"));
#ifndef _WIN32
    const int length = strlen(WEBOTS_INSTANCE_PATH) + strlen(robot_name) +
                       15; // "%sintern/%s/socket"
    socket_filename = malloc(length);
    snprintf(socket_filename, length, "%sipc/%s/intern", WEBOTS_INSTANCE_PATH,
             robot_name);
#else
    int i = 0;
    int last = -1;
    ...
    assert(number != -1);
    const int length =
        28 + strlen(robot_name); // "\\.\pipe\webots-XXXX-robot_name"
    socket_filename = malloc(length);
    snprintf(socket_filename, length, "\\\\.\\pipe\\webots-%d-%s", number,
             robot_name);
#endif
    free(robot_name);
  ...
  if (username == NULL || username[0] == '\0') {
    ...
    if (username == NULL || username[0] == '\0') {
      fprintf(stderr, "Error: USER or USERNAME environment variable not set, "
                      "falling back to 'default' username.\n");
      username = "default";
    }
  }
#endif
  ...
  char *WEBOTS_CONTROLLER_URL =
      (char *)wbu_system_getenv("WEBOTS_CONTROLLER_URL");
  // either the WEBOTS_CONTROLLER_URL is not defined, empty or contains only a
  // robot name default to the most recent /tmp/webots/username/* folder
  // (/tmp/webots-* on Windows)
  if (WEBOTS_CONTROLLER_URL == NULL || WEBOTS_CONTROLLER_URL[0] == 0 ||
      strstr(WEBOTS_CONTROLLER_URL, "://") == NULL) {
#ifndef _WIN32
    const int WEBOTS_TMP_DIR_length = strlen(TMP_DIR) + strlen(username) +
                                      9; // TMP_DIR + '/webots/' + username
    char *WEBOTS_TMP_DIR = malloc(WEBOTS_TMP_DIR_length);
    snprintf(WEBOTS_TMP_DIR, WEBOTS_TMP_DIR_length, "%s/webots/%s", TMP_DIR,
             username);
#else
    ...
    if (dr == NULL) {
      snprintf(error_buffer, ERROR_BUFFER_SIZE, "Cannot open directory %s",
               WEBOTS_TMP_DIR);
      free(WEBOTS_TMP_DIR);
      return NULL;
    }
...
#else
          double ts = filestat.st_mtim.tv_sec +
                      (filestat.st_mtim.tv_nsec /
                       1000000000.0); // last modification time
#endif
          // printf("ts = %.17lg\n", ts);
          if (ts > timestamp) {
            timestamp = ts;
    ...
    if (WEBOTS_CONTROLLER_URL &&
        WEBOTS_CONTROLLER_URL[0]) { // only the robot name was provided in
  ...
  if (strncmp(WEBOTS_CONTROLLER_URL, "ipc://", 6) != 0) {
    fprintf(stderr,
            "Error: unsupported protocol in WEBOTS_CONTROLLER_URL: %s\n",
            WEBOTS_CONTROLLER_URL);
    exit(EXIT_FAILURE);
  }
  ...
  if (number == -1) {
    fprintf(stderr,
            "Error: invalid WEBOTS_CONTROLLER_URL: %s (missing or wrong port "
            "value)\n",
            WEBOTS_CONTROLLER_URL);
    exit(EXIT_FAILURE);
  }
  ...
  const int webots_instance_path_length =
      strlen(TMP_DIR) + 20; // TMP_DIR + "/webots-12345678901"
  const int ipc_path_length =
      webots_instance_path_length + 4; // TMP_DIR + "/webots-12345678901/ipc"
#else
  const int webots_instance_path_length =
      strlen(TMP_DIR) + strlen(username) +
      21; // TMP_DIR + '/webots/' + username + '/12345678901'
  const int ipc_path_length =
      webots_instance_path_length +
      4; // TMP_DIR + '/webots/' + username + '/12345678901/ipc'
#endif
  char *webots_instance_folder = malloc(webots_instance_path_length);
  char *ipc_folder = malloc(ipc_path_length);
#ifdef _WIN32
  snprintf(webots_instance_folder, webots_instance_path_length, "%s/webots-%d",
           TMP_DIR, number);
  snprintf(ipc_folder, ipc_path_length, "%s/ipc", webots_instance_folder);
#else
  snprintf(webots_instance_folder, webots_instance_path_length,
           "%s/webots/%s/%d", TMP_DIR, username, number);
  snprintf(ipc_folder, ipc_path_length, "%s/ipc", webots_instance_folder);
#endif
  ...
  char *loading_file_path = malloc(strlen(webots_instance_folder) +
                                   9); // webots_instance_folder + '/loading'
  int written = snprintf(loading_file_path, path_length, "%s/loading",
                         webots_instance_folder);
  if (written < 0 || (size_t)written >= path_length) {
    fprintf(stderr,
            "Error: Constructing loading_file_path failed or truncated.\n");
  ...
  robot_name = encode_robot_name(sub_string ? sub_string + 1 : NULL,
                                 strlen(ipc_folder) + strlen("//extern"));
  if (robot_name) {
#ifndef _WIN32
    ...
    if (dr == NULL) { // the ipc folder was not yet created
      free(ipc_folder);
      snprintf(error_buffer, ERROR_BUFFER_SIZE,
               "The Webots simulation has not yet started or there is no robot "
               "in the simulation");
      return NULL;
    }
    char **filenames = NULL;
    ...
    if (count == 0) {
      snprintf(error_buffer, ERROR_BUFFER_SIZE,
               "The Webots simulation has not yet started or there is no robot "
               "with an <extern> controller");
      socket_filename = NULL;
    } else if (count > 1) { // more than one extern controller in the current
                            // instance of Webots
      fprintf(stderr,
              "No robot name provided, Webots instance %d should have exactly "
              "one robot set with an <extern> controller.\n",
              number);
...
#ifndef _WIN32
      const int l = ipc_path_length + strlen(filenames[0] + 1) +
                    8; // folder + robot_name + "/extern"
      socket_filename = malloc(l);
      snprintf(socket_filename, l, "%s/%s/extern", ipc_folder, filenames[0]);
#else
      const int l =
          28 + strlen(filenames[0] + 1); // "\\.\pipe\webots-XXXX" + robot_name
      socket_filename = malloc(l);
      snprintf(socket_filename, l, "\\\\.\\pipe\\webots-%d-%s", number,
               filenames[0]);
#endif
    }
    for (int i = 0; i < count; i++)
...
static void compute_remote_info(char **host, int *port, char **robot_name) {
  const char *WEBOTS_CONTROLLER_URL =
      wbu_system_getenv("WEBOTS_CONTROLLER_URL");
  ...
  const int host_length =
      strlen(&WEBOTS_CONTROLLER_URL[6]) - strlen(url_suffix) + 1;
...
// do not use any buffer for the standard streams
  ...
  // signal(SIGINT, quit_controller);  // this signal is working on Windows when
  // Ctrl+C from cmd.exe.
#ifndef _WIN32
  ...
  while (true) {
    ...
    if ((WEBOTS_CONTROLLER_URL != NULL) &&
        !(WEBOTS_ROBOT_NAME && WEBOTS_ROBOT_NAME[0] && WEBOTS_TMP_PATH &&
          WEBOTS_TMP_PATH[0]) &&
        strncmp(WEBOTS_CONTROLLER_URL, "tcp://", 6) ==
            0) { // TCP URL given and not an intern controller
      ...
      is_simulation_loading =
          strncmp(error_message, "The Webots simulation world is not yet ready",
                  44) == 0;
      if (success) {
        free(host);
        free(robot_name);
    ...
    if (retry % 5 == 0 && retry != 50) {
      if (is_simulation_loading) {
        ...
        fprintf(stderr,
                "The specified robot (at %s) is not in the list of robots with "
                "<extern> controllers, retrying for another %d "
                "seconds...\n",
