...
static void wb_lidar_new(WbDevice *d, unsigned int id, int w, int h, double fov,
                         double camnear, double max_range, bool planar,
...
static void wb_lidar_read_answer(WbDevice *d, WbRequest *r) {
  ...
  case C_CONFIGURE:
    ...
    // camnear, max_range, planar);
    wb_lidar_new(d, uid, width, height, fov, camnear, max_range, planar,
...
const float *wb_lidar_get_layer_range_image(WbDeviceTag tag, int layer) {
  ...
  if (layer >= l->number_of_layers) {
    fprintf(stderr,
            "Error: %s() called with a 'layer' argument (%d) bigger or equal "
            "to the number of layers of this lidar (%d).\n",
...
const WbLidarPoint *wb_lidar_get_point_cloud(WbDeviceTag tag) {
  ...
  if (!l->point_cloud_enabled) {
    fprintf(stderr,
            "Error: %s() called for a lidar with point cloud disabled. Please "
            "use: wb_lidar_enable_point_cloud().\n",
            __FUNCTION__);
    return NULL;
  }
  if (wb_abstract_camera_get_sampling_period(lidar_get_device(tag)) <= 0) {
