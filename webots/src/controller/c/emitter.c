...
static Packet *packet_create(const void *data, int size, int channel,
                             double range) {
...
static void emitter_read_answer(WbDevice *d, WbRequest *r) {
  ...
  case C_CONFIGURE: {
    ...
    es->allowed_channels =
        (int *)malloc(es->allowed_channels_size * sizeof(int));
  ...
  case C_EMITTER_SET_ALLOWED_CHANNELS: {
    Emitter *es = d->pdata;
    es->allowed_channels_size = request_read_int32(r);
    es->allowed_channels = (int *)realloc(
        es->allowed_channels, es->allowed_channels_size * sizeof(int));
    for (int i = 0; i < es->allowed_channels_size; i++)
      es->allowed_channels[i] = request_read_int32(r);
    break;
  }
...
static void emitter_write_request(WbDevice *d, WbRequest *r) {
  ...
  while (es->queue &&
         (es->byte_rate == -1.0 || es->queue->size <= es->bytes_to_send)) {
...
static void emitter_cleanup(WbDevice *d) { emitter_destroy(d->pdata); }
