...
static void handleDisplayMessage(WbRequest *r, WbDeviceTag tag,
                                 unsigned char c);
...
void remote_control_init(const char *library_name) {
  ...
  if (!_wbr_init || !_wbr_cleanup) {
    fprintf(stderr,
            "Error: %s remote control library entry points badly defined\n",
            library_name);
    remote_control_cleanup();
    return;
  }
  ...
  if (!check_remote_interface()) {
    fprintf(stderr,
            "Error: a mandatory function was not set in the WbrInterface of "
            "the %s remote control library\n",
            library_name);
    remote_control_cleanup();
    return;
  }
...
bool remote_control_is_initialized() { return initialized; }

bool remote_control_is_function_defined(const char *function_name) {
  return initialized &&
         dynamic_library_get_symbol(library_handle, function_name);
}
...
void remote_control_stop() { remoteInterface.mandatory.wbr_stop(); }
...
WbRequest *remote_control_handle_messages(WbRequest *r) {
  ...
  while (request_is_over(r) == 0) {
    ...
    if (robot_wwi_message || type == WB_NODE_DISPLAY ||
        type == WB_NODE_EMITTER || type == WB_NODE_RECEIVER) {
...
static void handleMessage(WbRequest *r, WbDeviceTag tag, WbNodeType type) {
  ...
  if (type == WB_NODE_ROTATIONAL_MOTOR || type == WB_NODE_LINEAR_MOTOR) {
    ...
    case C_MOTOR_SET_POSITION:
      CALL_INTERFACE_FUNCTION(wbr_motor_set_position, tag,
                              request_read_double(r));
      break;
    case C_MOTOR_SET_VELOCITY:
      CALL_INTERFACE_FUNCTION(wbr_motor_set_velocity, tag,
                              request_read_double(r));
      break;
    case C_MOTOR_SET_ACCELERATION:
      CALL_INTERFACE_FUNCTION(wbr_motor_set_acceleration, tag,
                              request_read_double(r));
      break;
    case C_MOTOR_SET_FORCE: {
      if (type == WB_NODE_LINEAR_MOTOR)
        CALL_INTERFACE_FUNCTION(wbr_motor_set_force, tag,
                                request_read_double(r));
      else
        CALL_INTERFACE_FUNCTION(wbr_motor_set_torque, tag,
                                request_read_double(r));
      break;
    }
    case C_MOTOR_SET_AVAILABLE_FORCE: {
      if (type == WB_NODE_LINEAR_MOTOR)
        CALL_INTERFACE_FUNCTION(wbr_motor_set_available_force, tag,
                                request_read_double(r));
      else
        CALL_INTERFACE_FUNCTION(wbr_motor_set_available_torque, tag,
                                request_read_double(r));
      break;
    }
    ...
    case C_MOTOR_FEEDBACK: {
      if (type == WB_NODE_LINEAR_MOTOR)
        CALL_INTERFACE_FUNCTION(wbr_motor_set_force_sampling_period, tag,
                                request_read_uint16(r));
      else
        CALL_INTERFACE_FUNCTION(wbr_motor_set_torque_sampling_period, tag,
                                request_read_uint16(r));
      break;
    }
  ...
  case WB_NODE_TOUCH_SENSOR:
    if (c == C_SET_SAMPLING_PERIOD)
      CALL_INTERFACE_FUNCTION(wbr_set_sampling_period, tag,
                              request_read_uint16(r));
    else {
      ...
      case WB_NODE_CAMERA:
        ...
        case C_CAMERA_SET_FOV:
          CALL_INTERFACE_FUNCTION(wbr_camera_set_fov, tag,
                                  request_read_double(r));
          break;
        case C_CAMERA_SET_EXPOSURE:
          CALL_INTERFACE_FUNCTION(wbr_camera_set_exposure, tag,
                                  request_read_double(r));
          break;
        case C_CAMERA_SET_FOCAL:
          CALL_INTERFACE_FUNCTION(wbr_camera_set_focal_distance, tag,
                                  request_read_double(r));
          break;
        default:
  ...
  case WB_NODE_SPEAKER:
    ...
    CALL_INTERFACE_FUNCTION(wbr_speaker_emit_sample, tag,
                            request_read_data(r, size), size);
    break;
...
static void handleDisplayMessage(WbRequest *r, WbDeviceTag tag,
  ...
  case C_DISPLAY_SET_ALPHA:
    CALL_INTERFACE_FUNCTION(wbr_display_set_alpha, tag,
                            request_read_uchar(r) / 255.0);
    break;
  case C_DISPLAY_SET_OPACITY:
    CALL_INTERFACE_FUNCTION(wbr_display_set_opacity, tag,
                            request_read_uchar(r) / 255.0);
    break;
  default:
    if (c & 0x20) { // draw message
      ...
      case C_DISPLAY_DRAW_LINE:
        ROBOT_ASSERT(size == 2);
        CALL_INTERFACE_FUNCTION(wbr_display_draw_line, tag, x[0], y[0], x[1],
                                y[1]);
        break;
      ...
      case C_DISPLAY_DRAW_RECTANGLE:
        ...
        if (request_read_uchar(r))
          CALL_INTERFACE_FUNCTION(wbr_display_fill_rectangle, tag, x[0], y[0],
                                  x[1], y[1]);
        else
          CALL_INTERFACE_FUNCTION(wbr_display_draw_rectangle, tag, x[0], y[0],
                                  x[1], y[1]);
        break;
      case C_DISPLAY_DRAW_OVAL:
        ROBOT_ASSERT(size == 2);
        if (request_read_uchar(r))
          CALL_INTERFACE_FUNCTION(wbr_display_fill_oval, tag, x[0], y[0], x[1],
                                  y[1]);
        else
          CALL_INTERFACE_FUNCTION(wbr_display_draw_oval, tag, x[0], y[0], x[1],
                                  y[1]);
        break;
      ...
      case C_DISPLAY_IMAGE_COPY:
        ...
        CALL_INTERFACE_FUNCTION(wbr_display_image_copy, tag, id, x, y, width,
                                height);
        break;
      ...
      case C_DISPLAY_IMAGE_LOAD:
        ...
        CALL_INTERFACE_FUNCTION(
            wbr_display_image_new, tag, id, width, height,
            request_read_data(r,
                              width * height * display_get_channel_number(x)),
            x);
        break;
      ...
      case C_DISPLAY_IMAGE_GET_ALL: {
        ...
        CALL_INTERFACE_FUNCTION(wbr_display_image_new, tag, -1, width, height,
                                im, WB_IMAGE_RGBA);
        CALL_INTERFACE_FUNCTION(wbr_display_image_paste, tag, -1, 0, 0);
        CALL_INTERFACE_FUNCTION(wbr_display_image_delete, tag, -1);
        int i;
        int number =
            id; // in this message the first "id" is the number of imageRef
        for (i = 0; i < number; i++) {
        ...
        CALL_INTERFACE_FUNCTION(wbr_display_set_color, tag,
                                request_read_int32(r));
        CALL_INTERFACE_FUNCTION(wbr_display_set_alpha, tag,
                                request_read_uchar(r) / 255.0);
        CALL_INTERFACE_FUNCTION(wbr_display_set_opacity, tag,
                                request_read_uchar(r) / 255.0);
