...
int Robot::step(int duration) { return wb_robot_step(duration); }

int Robot::stepBegin(int duration) { return wb_robot_step_begin(duration); }

int Robot::stepEnd() { return wb_robot_step_end(); }

Robot::UserInputEvent Robot::waitForUserInputEvent(UserInputEvent event_type,
                                                   int timeout) {
  return UserInputEvent(wb_robot_wait_for_user_input_event(
      WbUserInputEvent(event_type), timeout));
}

string Robot::getName() const { return string(wb_robot_get_name()); }

string Robot::getUrdf(string prefix) const {
  return string(wb_robot_get_urdf(prefix.c_str()));
}

double Robot::getTime() const { return wb_robot_get_time(); }

string Robot::getModel() const { return string(wb_robot_get_model()); }
...
string Robot::getData() const {
  fprintf(stderr, "Robot::getData is deprecated, please use "
                  "Robot::getCustomData instead\n");
  return string(wb_robot_get_custom_data());
}

void Robot::setData(const std::string &data) {
  fprintf(stderr, "Robot::setData is deprecated, please use "
                  "Robot::setCustomData instead\n");
  wb_robot_set_custom_data(data.c_str());
}

Robot::Mode Robot::getMode() const { return Mode(wb_robot_get_mode()); }

void Robot::setMode(Mode m, const char *arg) {
  wb_robot_set_mode(WbRobotMode(m), arg);
}

bool Robot::getSupervisor() const { return wb_robot_get_supervisor(); }
...
string Robot::getWorldPath() const { return string(wb_robot_get_world_path()); }
...
void Robot::batterySensorDisable() { wb_robot_battery_sensor_disable(); }
...
Brake *Robot::createBrake(WbDeviceTag tag) const { return new Brake(tag); }
...
Camera *Robot::createCamera(WbDeviceTag tag) const { return new Camera(tag); }
...
GPS *Robot::createGPS(WbDeviceTag tag) const { return new GPS(tag); }
...
Gyro *Robot::createGyro(WbDeviceTag tag) const { return new Gyro(tag); }
...
LED *Robot::createLED(WbDeviceTag tag) const { return new LED(tag); }
...
Lidar *Robot::createLidar(WbDeviceTag tag) const { return new Lidar(tag); }
...
Motor *Robot::getMotor(const string &name) {
  WbDeviceTag tag = wb_robot_get_device(name.c_str());
  if (!Device::hasType(tag, WB_NODE_LINEAR_MOTOR) &&
      !Device::hasType(tag, WB_NODE_ROTATIONAL_MOTOR))
    return NULL;
  return dynamic_cast<Motor *>(getOrCreateDevice(tag));
}

Motor *Robot::createMotor(WbDeviceTag tag) const { return new Motor(tag); }
...
Pen *Robot::createPen(WbDeviceTag tag) const { return new Pen(tag); }
...
Radar *Robot::createRadar(WbDeviceTag tag) const { return new Radar(tag); }
...
Skin *Robot::createSkin(WbDeviceTag tag) const { return new Skin(tag); }
...
Device *Robot::getOrCreateDevice(int tag) {
  ...
  // deleted devices are not removed from the C API list and don't affect the
  // number of devices
...
const char *Robot::wwiReceive(int *size) { return wb_robot_wwi_receive(size); }
