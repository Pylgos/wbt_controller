...
#define wb_camera_image_get_red(image, width, x, y)                            \
  (image[4 * ((y) * (width) + (x)) + 2])
#define wb_camera_image_get_green(image, width, x, y)                          \
  (image[4 * ((y) * (width) + (x)) + 1])
#define wb_camera_image_get_blue(image, width, x, y)                           \
  (image[4 * ((y) * (width) + (x))])

#ifdef KROS_COMPILATION
#define wb_camera_image_get_gray(image, width, x, y)                           \
  (image[(y) * (width) + (x)])
#else
#define wb_camera_image_get_gray(image, w, x, y)                               \
  ((image[4 * ((y) * (w) + (x)) + 2] + image[4 * ((y) * (w) + (x)) + 1] +      \
    image[4 * ((y) * (w) + (x))]) /                                            \
   3)
#endif
// alias
#define wb_camera_image_get_grey(image, width, x, y)                           \
  wb_camera_image_get_gray(image, width, x, y)
