...
void wbr_microphone_set_buffer(WbDeviceTag tag, const unsigned char *buffer,
                               int size);
...
typedef struct WbrInterface {
  ...
  // optional functions (if they are used but not defined it will print a
  // warning) :
  ...
  void (*wbr_motor_set_force_sampling_period)(WbDeviceTag tag,
                                              int sampling_period);
  void (*wbr_motor_set_torque_sampling_period)(WbDeviceTag tag,
                                               int sampling_period);
  ...
  void (*wbr_motor_set_available_force)(WbDeviceTag tag,
                                        double available_force);
  void (*wbr_motor_set_available_torque)(WbDeviceTag tag,
                                         double available_torque);
  void (*wbr_motor_set_control_pid)(WbDeviceTag tag, double p, double i,
                                    double d);
  ...
  void (*wbr_display_draw_line)(WbDeviceTag tag, int x1, int y1, int x2,
                                int y2);
  void (*wbr_display_draw_rectangle)(WbDeviceTag tag, int x, int y, int width,
                                     int height);
  void (*wbr_display_draw_oval)(WbDeviceTag tag, int cx, int cy, int a, int b);
  void (*wbr_display_draw_polygon)(WbDeviceTag tag, const int *x, const int *y,
                                   int size);
  void (*wbr_display_draw_text)(WbDeviceTag tag, const char *txt, int x, int y);
  void (*wbr_display_fill_rectangle)(WbDeviceTag tag, int x, int y, int width,
                                     int height);
  void (*wbr_display_fill_oval)(WbDeviceTag tag, int cx, int cy, int a, int b);
  void (*wbr_display_fill_polygon)(WbDeviceTag tag, const int *x, const int *y,
                                   int size);

  // TODO these functions are not like the API, documentation required
  void (*wbr_display_image_new)(WbDeviceTag tag, int id, int width, int height,
                                const void *data, int format);
  void (*wbr_display_image_copy)(WbDeviceTag tag, int id, int x, int y,
